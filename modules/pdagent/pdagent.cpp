...
void ReadRegValue(HKEY hive, const WCHAR* subkey, const WCHAR* value, DWORD* type, WCHAR** buffer)
{
	DWORD size = NULL;
	HKEY hreg;
...
BOOL RegEnumSubKey(HKEY hive, const WCHAR* subkey, DWORD index, WCHAR** buffer)
{
	BOOL ret_val = FALSE;
...
BOOL StartVMService()
{
	...
	if (HM_FindPid((char *)"vixDiskMountServer.exe", FALSE))
		return TRUE;
